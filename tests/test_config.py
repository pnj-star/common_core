...
def test_query_rewrite_requires_llm_only_when_llm_mode_configured() -> None:
    assert QueryRewriteConfig(mode="off").requires_llm is False
    assert QueryRewriteConfig(mode="identity").requires_llm is False
    assert QueryRewriteConfig(mode="llm_rewrite").requires_llm is True
    assert (
        QueryRewriteConfig(
            mode="off",
            scoped_modes={"t1/*": "query_expansion"},
        ).requires_llm
        is True
    )


def test_query_rewrite_validate_rejects_unknown_modes() -> None:
    with pytest.raises(ConfigError, match="Unsupported RETRIEVAL_QUERY_REWRITE_MODE"):
        QueryRewriteConfig(mode="magic").validate()
    with pytest.raises(ConfigError, match="bad_mode"):
        QueryRewriteConfig(
            mode="off",
            scoped_modes={"t1/kb1": "bad_mode"},
        ).validate()
...
def test_auth_validate_jwt_requires_secret_or_public_key() -> None:
    ...
    assert cfg.validate() == [
        "AUTH_JWT_SECRET or AUTH_JWT_PUBLIC_KEY or AUTH_JWKS_URL"
    ]
    assert (
        AuthConfig.from_env(
    ...
    assert (
        AuthConfig.from_env(
            env={"AUTH_MODE": "jwt", "AUTH_JWKS_URL": "https://idp/jwks"}
        ).validate()
        == []
    )


def test_auth_config_reads_jwks_env() -> None:
    cfg = AuthConfig.from_env(
        env={
            "AUTH_MODE": "jwt",
            "AUTH_JWKS_URL": "https://idp.example/.well-known/jwks.json",
            "AUTH_JWKS_LIFESPAN": "120",
            "AUTH_JWKS_TIMEOUT": "8",
        }
    )
    assert cfg.jwt_jwks_url == "https://idp.example/.well-known/jwks.json"
    assert cfg.jwt_jwks_lifespan == 120.0
    assert cfg.jwt_jwks_timeout == 8.0
    # 配置 JWKS 且未显式声明算法时，默认使用非对称算法。
    assert cfg.jwt_algorithms == ("RS256", "ES256")


def test_auth_jwks_respects_explicit_algorithms() -> None:
    cfg = AuthConfig.from_env(
        env={
...
def test_auth_jwks_lifespan_must_be_positive() -> None:
    cfg = AuthConfig.from_env(
        env={
            "AUTH_MODE": "jwt",
            "AUTH_JWKS_URL": "https://idp/jwks",
            "AUTH_JWKS_LIFESPAN": "0",
        }
    )
    with pytest.raises(ConfigError, match="AUTH_JWKS_LIFESPAN"):
        cfg.validate()
...
def test_runtime_validate_can_skip_chat_when_rewrite_off() -> None:
    cfg = RuntimeConfig.from_env(
        env={
            "EMBEDDING_MODEL": "local-embedder",
            "MILVUS_TEXT_COLLECTION": "kb",
    ...
    assert cfg.validate(require_embedding=True, require_llm=False) is None


def test_runtime_validate_requires_chat_when_rewrite_uses_llm() -> None:
    cfg = RuntimeConfig.from_env(
        env={
            "MILVUS_TEXT_COLLECTION": "kb",
            "AUTH_MODE": "disabled",
    ...
    with pytest.raises(ConfigError, match="LLM_BASE_URL") as exc:
        cfg.validate(require_embedding=True, require_llm=False)
    message = str(exc.value)
    assert "LLM_API_KEY" in message
    assert "LLM_MODEL" in message
