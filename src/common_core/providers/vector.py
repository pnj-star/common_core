...
参数:
    ...
    def connect(self) -> None:
        """建立与 Milvus 的连接（仅此一次），使用官方推荐的 MilvusClient。
        """
    ...
    def _ensure(self, collection_name: str) -> Any:
        """确保集合已加载并返回 MilvusClient，缺少时自动连接并加载。
参数:
    collection_name: 要访问的集合名称。
返回:
    已连接并确保集合加载完成的 MilvusClient 实例。
异常:
    ValueError: 集合不存在时抛出。
        """
        client = self._client
        if collection_name in self._loaded_collections and client is not None:
            return client
        self.connect()
        client = self._client
        if client is None:
            ...
            for hit in hits:
                if isinstance(hit, dict):
                    item: dict[str, Any] = {
                        ...
                        score_key: float(hit.get("distance", 0.0) or 0.0),
                    }
...
返回:
    字典列表。
        """
        client = self._ensure(collection_name)
    ...
    def close(self) -> None:
        ...
        if self._client is not None:
            try:
                ...
                logger.debug("milvus close failed (ignored)", exc_info=True)
            self._client = None
