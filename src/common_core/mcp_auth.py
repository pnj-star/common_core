...
class ToolContextGuard:
    ...
    def __post_init__(self) -> None:
        """未显式注入 verifier 时，按当前 AuthConfig 自动构建 TokenVerifier。

        TokenVerifier 会读取 config 中的静态密钥 / 公钥，或 AUTH_JWKS_URL
        指向的 JWKS 端点，保证工具鉴权与传输层鉴权使用同一份密钥来源。
        """
...
class MCPBearerTokenVerifier:
    ...
    如果处于鉴权关闭模式，则返回 None。这样 HTTP 部署场景，即便 MCP 进程
    没有配置静态公钥 / 密钥，只要配置了 AUTH_JWKS_URL，传输层仍可自动拉取
    IdP 公钥并按 TTL 刷新，密钥轮换无需重启。
    """
