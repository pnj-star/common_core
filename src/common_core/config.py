...
VALID_QUERY_REWRITE_MODES = frozenset(
    {"off", "llm_rewrite", "query_expansion"}
)
_LLM_QUERY_REWRITE_MODES = frozenset({"llm_rewrite", "query_expansion"})
...
class LLMConfig:
    ...
    def validate(
        ...
        参数:
            ...
            require_chat: 为 True 时要求必须配置对话模型三件套。

        返回:
            缺失配置键的名称列表；全部完整时为空列表。
        """
        missing: list[str] = []
        if require_chat and not self.base_url:
            missing.append(f"{prefix}BASE_URL")
        if require_chat and not self.api_key:
            missing.append(f"{prefix}API_KEY")
        if require_chat and not self.model:
...
@dataclass(slots=True)
class AuthConfig:
    """JWT 校验配置。支持静态密钥（HS256）或公钥（RS256/ES256），
    也支持通过 ``jwt_jwks_url`` 指向 IdP 的 JWKS 端点自动拉取公钥，
    按 TTL 缓存并在密钥轮换后自动刷新，无需重启服务进程。
    密钥与 JWKS 均为空时，由调用方自行决定校验策略。
    """
    ...
    jwt_audience: str = ""
    jwt_jwks_url: str = ""
    jwt_jwks_lifespan: float = 300.0
    jwt_jwks_timeout: float = 30.0
    mcp_issuer_url: str = ""
    ...
    def from_env(
        ...
        """
        raw_algorithms = env_str(f"{prefix}JWT_ALGORITHMS", env=env)
        jwks_url = env_str(f"{prefix}JWKS_URL", env=env)
        return cls(
            ...
            # 配置了 JWKS 端点时，默认使用非对称算法；显式声明算法时以声明为准。
            jwt_algorithms=(
                tuple(env_list(f"{prefix}JWT_ALGORITHMS", env=env))
                if raw_algorithms.strip()
                else (("RS256", "ES256") if jwks_url else ("HS256",))
            ),
            jwt_issuer=env_str(f"{prefix}JWT_ISSUER", env=env),
            jwt_audience=env_str(f"{prefix}JWT_AUDIENCE", env=env),
            jwt_jwks_url=jwks_url,
            jwt_jwks_lifespan=env_float(
                f"{prefix}JWKS_LIFESPAN", default=300.0, env=env
            ),
            jwt_jwks_timeout=env_float(
                f"{prefix}JWKS_TIMEOUT", default=30.0, env=env
            ),
    ...
    def validate(self, prefix: str = "AUTH_") -> list[str]:
        ...
        if (
            ...
            and not self.jwt_public_key
            and not self.jwt_jwks_url
        ):
            missing.append(
                f"{prefix}JWT_SECRET or {prefix}JWT_PUBLIC_KEY or {prefix}JWKS_URL"
            )
        if self.mode == "jwt" and self.jwt_jwks_url and self.jwt_jwks_lifespan <= 0:
            raise ConfigError(f"{prefix}JWKS_LIFESPAN must be greater than 0")
        return missing
...
class RetrievalConfig:
    ...
    def from_env(
        ...
        return cls(
            ...
            # 重排相关键采用独立前缀 RERANKER_（与 retrieve_skill 的 .env 约定一致）。
...
class QueryRewriteConfig:
    ...
    mode 支持 off / llm_rewrite / query_expansion，默认 off；
    ...
    def requires_llm(self) -> bool:
        """当前默认模式或任何作用域覆盖是否需要调用 LLM。"""
        return self.mode in _LLM_QUERY_REWRITE_MODES or any(
            mode in _LLM_QUERY_REWRITE_MODES for mode in self.scoped_modes.values()
        )

    def validate(self, prefix: str = "RETRIEVAL_QUERY_REWRITE_") -> list[str]:
        """校验改写模式合法；非法时抛 ConfigError，否则返回空列表。"""
        if self.mode not in VALID_QUERY_REWRITE_MODES:
            raise ConfigError(
                f"Unsupported {prefix}MODE={self.mode!r}; expected "
                "off/llm_rewrite/query_expansion"
            )
        for scope, mode in self.scoped_modes.items():
            normalized = mode.strip().lower()
            if normalized not in VALID_QUERY_REWRITE_MODES:
                raise ConfigError(
                    f"Unsupported {prefix}SCOPES mode {mode!r} for scope {scope!r}; "
                    "expected off/llm_rewrite/query_expansion"
                )
        return []
...
class RuntimeConfig:
    ...
    def validate(self, *, require_embedding: bool = False, require_llm: bool = True) -> None:
        """当必需的运行时配置缺失时快速失败。
        ...
        参数:
            require_embedding: 为 True 时要求必须配置 embedding 模型。
            require_llm: 业务默认要求 LLM；False 时仅在查询改写实际启用
                LLM 模式（默认或作用域覆盖）才要求对话模型三件套。

        异常:
            ConfigError: 任一必需配置键缺失时抛出。
        """
        missing: list[str] = []
        missing.extend(self.query_rewrite.validate())
        missing.extend(
            self.llm.validate(
                require_embedding=require_embedding,
                require_chat=require_llm or self.query_rewrite.requires_llm,
            )
        )
...
def config_snapshot(runtime: RuntimeConfig) -> list[tuple[str, str]]:
    ...
    for key in ("mode", "jwt_secret", "jwt_public_key", "jwt_jwks_url"):
...
def log_config_audit(
    ...
    *,
    ...
    参数:
        ...
        require_llm: 是否要求 LLM 三件套；False 时按查询改写配置决定。
    ...
    runtime.validate(require_embedding=require_embedding, require_llm=require_llm)
